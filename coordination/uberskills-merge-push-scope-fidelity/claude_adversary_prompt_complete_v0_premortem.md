# Claude adversarial review prompt — scope fidelity + V0 premortem update

You are reviewing Codex changes. Do not review a narrowed summary. Section 1 is the durable scope artifact with the operator original instruction and dated scope additions. Section 2 is the actual artifact/diff under review.

Focus especially on the latest scope addition: add a durable uberplan V0 premortem / failure-disposition gate for Tier 2+ plans, include overengineering/code-bloat failure analysis, include Claude's recs that adversary failure questions become load-bearing, and keep the change surgical with no new top-level skill and no semantic judge.

## Review questions

1. Does the latest uberplan change implement an explicit post-v0 premortem gate before confidence/implementation?
2. Does it include overengineering/code-bloat, files/modules/abstractions, delete/merge/avoid, and 80/50 alternative coverage?
3. Are failure dispositions load-bearing: plan revision or accepted-risk rationale required?
4. Is the validator structural/presence/order only, avoiding semantic plan-quality judgment?
5. Is the change surgical, or did it introduce broad skill/process bloat?
6. Name any blocker that would make this unsafe to merge.
...
+    verdict = require_field(section, "Premortem gate verdict", errors).lower()
+    if verdict and not verdict.startswith("pass"):
+        errors.append(f"V0 plan premortem verdict is not pass: {verdict}")
+
+
...
index 047cc32..bb3cbb0 100644
--- a/uberplan/tests/test_validators.py
+++ b/uberplan/tests/test_validators.py
@@ -75,6 +75,28 @@ class PlanValidatorTests(unittest.TestCase):
...
@@ -167,6 +189,17 @@ class PlanValidatorTests(unittest.TestCase):
             plan.write_text(text[:start] + text[end:])
             self.assertFails(str(PLAN), str(plan), "--tier", "2", "--agent-behavior")
