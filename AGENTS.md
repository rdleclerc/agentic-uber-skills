...
- `uberarchitect` = architecture stepback gate. Use it directly or route to it from `uberassess`, `uberplan`, `ubergoal`, or `uberaccept` for concurrency, scale, queue/worker, long-running job, gateway, orchestration, backpressure, repeated-timeout, or symptom-patching failures before local implementation scope hardens.
...
- `testing-strategy` = automatic portable test-selection utility. It may run inside or outside `$ubergoal` whenever work includes a test or product-validation decision; it does not own lifecycle routing, authorize a change, or replace acceptance.
...
- For architecture-shaped failures, `$uberarchitect` must produce a plain-English Architecture Stepback Packet before the workflow accepts local micro-patches as the answer.
...
- `uberplan` produces the smallest source-grounded plan that completely addresses the evidenced problem. A contained plan may be inline; create a durable `.md` plan when the operator requests it or execution is long-running, resumable, multi-owner, or needs a handoff. Thread highlights, Mermaid maps, plan trees, and full templates are evidence-gated, not default output.
