#!/usr/bin/env python3
"""Lint portable SKILL.md package shape without mutating files."""
...
def add_overlap_findings(results: list[dict[str, Any]]) -> None:
    ...
    for index, (left, left_words) in enumerate(descriptions):
        ...
        for right, right_words in descriptions[index + 1 :]:
            ...
            if similarity >= 0.42:
                overlaps.append({"skill": right["name"], "similarity": round(similarity, 2)})
                right.setdefault("overlap_candidates", []).append({"skill": left["name"], "similarity": round(similarity, 2)})
        if overlaps:
...
def main() -> int:
    ...
    parser.add_argument("paths", nargs="+", type=Path, help="Skill directory or pack root to lint")
    ...
    return 1 if report["summary"]["severity_counts"].get("error", 0) else 0
