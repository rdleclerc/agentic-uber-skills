...
class ThinWrapperTests(unittest.TestCase):
    ...
    def test_skill_body_is_thin_and_routes(self) -> None:
        ...
        self.assertIn("Effort scales with tier", text)
        self.assertIn("xhigh only where a smaller slice cannot preserve safety", text)
        self.assertIn("smaller slice cannot preserve safety", text)
        self.assertIn("policy-adherence", text)
        self.assertIn("ready_for_acceptance", text)
        self.assertIn("one independent risk-specific lane", text)
        self.assertIn("templates/uber-run-receipt.md", text)
